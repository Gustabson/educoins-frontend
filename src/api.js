...
const api = {
  ...
  updateMission:  (id, data)              => apiFetch(`/missions/${id}`,      { method:"PATCH",  body:data }),
  deleteMission:  (id)                    => apiFetch(`/missions/${id}`,      { method:"DELETE" }),
  storeItems:     ()                      => apiFetch("/store/items"),
  myStoreItems:   ()                      => apiFetch("/store/items/mine"),
  reportStoreItem:(id, reason)            => apiFetch(`/store/items/${id}/report`, { method:"POST", body:{reason} }),
  ...
  missionRewardAll: (id)     => apiFetch(`/missions/${id}/reward-all`, { method:"POST" }),
  // ── Peer Evaluation / Groups ──────────────────────────────
  peerClassmates:     (missionId) => apiFetch(`/peer-eval/classmates?mission_id=${missionId}`),
  peerCreateGroup:    (d)         => apiFetch("/peer-eval/groups",            { method:"POST", body:d }),
  peerAcceptGroup:    (id)        => apiFetch(`/peer-eval/groups/${id}/accept`,{ method:"POST" }),
  peerLeaveGroup:     (id)        => apiFetch(`/peer-eval/groups/${id}/leave`, { method:"POST" }),
  peerMyGroups:       ()          => apiFetch("/peer-eval/groups/mine"),
  peerPending:        ()          => apiFetch("/peer-eval/pending"),
  peerEvaluate:       (d)         => apiFetch("/peer-eval/evaluate",          { method:"POST", body:d }),
  peerDashboard:      (q)         => apiFetch(`/peer-eval/dashboard${q?`?${q}`:""}`),
  peerStudent:        (id)        => apiFetch(`/peer-eval/student/${id}`),
  peerTeacherObs:     (d)         => apiFetch("/peer-eval/teacher-observation",{ method:"POST", body:d }),
  peerTriangulation:  ()          => apiFetch("/peer-eval/triangulation"),
  peerRankingConfig:  ()          => apiFetch("/peer-eval/ranking/config"),
  peerRankingUpsert:  (d)         => apiFetch("/peer-eval/ranking/config",    { method:"POST", body:d }),
  peerRankingLive:    ()          => apiFetch("/peer-eval/ranking/live"),
  peerRankingClose:   ()          => apiFetch("/peer-eval/ranking/close",     { method:"POST" }),
  peerRankingPayouts: ()          => apiFetch("/peer-eval/ranking/payouts"),
  peerConfig:         ()          => apiFetch("/peer-eval/config"),
  peerConfigUpdate:   (d)         => apiFetch("/peer-eval/config",            { method:"PATCH", body:d }),
  // ── Chat ──────────────────────────────────────────────────
  ...
  diwyParentInstantReport: (studentId) => apiFetch('/diwy/parent/instant-report', { method:"POST", body:{ studentId } }),
  diwyChildTeachers:      (studentId) => apiFetch(`/diwy/parent/child-teachers?studentId=${studentId}`),
  diwyParentDirectMsg:    (data)      => apiFetch('/diwy/parent/direct-message', { method:'POST', body: data }),
  diwyParentDirectThread: (p)         => apiFetch(`/diwy/parent/direct-messages?studentId=${p.studentId}&teacherId=${p.teacherId}`),
