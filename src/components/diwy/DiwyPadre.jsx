// DiwyPadre.jsx — Parent view of Diwy.
// Access: parent role only (enforced server-side).
// Can ONLY read: approved reports for their own children.
// Backend filters by JWT parent_id → parent_student_links. No student data leaks.
...
export default function DiwyPadre({ showToast, onBack }) {
  ...
  useEffect(() => {
    // Only fetches reports the backend authorizes for this parent.
  ...
  const requestReport = async (studentId) => {
    ...
    try {
      ...
      setRequesting(p => ({ ...p, [studentId]: false }));
    }
  };
        ...
        {!loading && Object.keys(byStudent).length === 0 && (
          <WCard style={{ textAlign:"center", padding:32 }}>
            ...
            <div style={{ fontSize:13, color:sub, lineHeight:1.6, transition:"color .3s" }}>
              Diwy genera reportes personalizados sobre el progreso de tus hijos.
        ...
        {Object.entries(byStudent).map(([studentId, { nombre, reports: sr }]) => {
          ...
          return (
            <div key={studentId} style={{ marginBottom:16 }}>
              ...
              {latest && (
                <WCard style={{ marginBottom:8 }}>
                  ...
                  <div style={{ fontSize:13, color:txt, lineHeight:1.6,
                    overflow:"hidden", maxHeight: isExp ? "none" : 80,
                    position:"relative", transition:"color .3s" }}>
                    <div style={{ whiteSpace:"pre-wrap" }}>{latest.reporte_final}</div>
                    {!isExp && (
                      <div style={{ position:"absolute", bottom:0, left:0, right:0,
                        height:40, background:`linear-gradient(transparent, ${cardBg})`,
              ...
              <button onClick={() => requestReport(studentId)}
                disabled={requesting[studentId]}
                style={{ width:"100%", background: requesting[studentId] ? navBord : `${primary}18`,
                  border:`1.5px dashed ${primary}55`, borderRadius:14, padding:"12px",
                  cursor: requesting[studentId] ? "not-allowed" : "pointer",
                  fontFamily:"Nunito,sans-serif", color:primary, fontWeight:800, fontSize:13,
