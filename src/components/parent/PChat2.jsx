...
const SECTIONS = ["🌐 Global", "👥 Grupos", "🏫 Aula", "💬 Privado"];
const GROUP_ICONS = ["👥","👨‍👩‍👧","🏠","📚","🌟","🎉","💡","🤝","🏆","🌈","🎲","🔥"];
...
export default function PChat2({ me, showToast, onBack }) {
  ...
  const socketRef = usePChatSocket(token, onMessage, onTyping);
  ...
  useEffect(() => {
    Promise.all([
      ...
      setClassInfo(clData);
      if (clData?.conversation_id) {
        classConvIdRef.current = clData.conversation_id;
      }
  ...
  // Recargar grupos al entrar a la tab
  useEffect(() => {
    if (sec === 1) {
      api.myGroups().then(d => {
        const gs = d || [];
  ...
  // Búsqueda de padres con debounce
  useEffect(() => {
  ...
  const rejectFriend = async id => {
    try {
      ...
      setPend(p => p.filter(f => f.friendship_id !== id));
    } catch {}
  };
  ...
  // ── Render: mensaje burbuja ─────────────────────────────────
  const MsgBubble = ({ m, idx }) => {
    const isMe = m.sender_id === me.id;
    return (
      <div key={m.id || idx} style={{ display:"flex", justifyContent:isMe?"flex-end":"flex-start",
        gap:8, alignItems:"flex-end" }}>
        {!isMe && <Av user={{ skin:m.skin, border:m.border, nombre:m.sender_nombre,
          avatar_bg:m.avatar_bg||null, foto_url:m.foto_url||null }} sz={28} avatarBg={m.avatar_bg||null}/>}
        <div style={{ maxWidth:"72%" }}>
          {!isMe && <div style={{ fontSize:10, marginBottom:2, marginLeft:4, fontWeight:700, color:sub }}>
            {m.sender_nombre}
          </div>}
  ...
  // ── Render: sala de mensajes (Global / Aula) ─────────────────
  const renderRoom = (msgs, emptyMsg) => (
    <div style={{ display:"flex", flexDirection:"column",
      height:"calc(100vh - 180px)", position:"relative" }}>
      <div style={{ flex:1, overflowY:"auto", padding:"10px 14px 80px",
        display:"flex", flexDirection:"column", gap:8 }}>
        {msgs.length === 0 && (
          <div style={{ textAlign:"center", color:sub, fontSize:13, marginTop:40 }}>{emptyMsg}</div>
        )}
      ...
      <div style={{ position:"absolute", bottom:0, left:0, right:0,
        padding:"6px 14px 16px", background:cardBg, borderTop:`1px solid ${inputBg}` }}>
  ...
  // ── Render: chat de grupo (idéntico a AChat) ──────────────────
  if (activeGroup) {
    ...
    return (
      <div style={{ background:pageBg, height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={{ background:accent, color:"white", flexShrink:0, position:"sticky", top:0, zIndex:10 }}>
            ...
            <button onClick={() => {
              ...
              if (opening && canInvite) {
                Promise.all([api.chatFriends(), api.groupMembers(activeGroup.conversation_id)])
                  .then(([fr, mem]) => {
                    const memberIds = new Set((mem||[]).map(m => m.user_id));
                    setGroupFriends((fr||[]).filter(f => !memberIds.has(f.user_id)));
                  }).catch(() => {});
              }
            }}
          ...
          {groupPanel && (
            <div style={{ background:"rgba(0,0,0,.25)", backdropFilter:"blur(8px)",
              ...
              {canInvite && (
                <div>
                  <div style={{ fontSize:10, fontWeight:800, opacity:.7, marginBottom:6 }}>AGREGAR AMIGOS AL GRUPO</div>
        ...
        <div style={{ flex:1, padding:"12px 14px 12px", overflowY:"auto",
          display:"flex", flexDirection:"column", gap:8 }}>
  ...
  // ── Render: modal "Buscar / Agregar amigo" ──────────────────
  if (addOpen) return (
    <div style={{ background:pageBg, minHeight:"100vh", fontFamily:"Nunito,sans-serif" }}>
      ...
      <div style={{ padding:"14px" }}>
          ...
          <div key={u.id} style={{ display:"flex", alignItems:"center", gap:10,
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              {u.apodo && u.apodo !== u.nombre && (
                <div style={{ fontSize:11, color:sub }}>({u.nombre})</div>
              )}
            </div>
        ...
        {search.length >= 2 && results.length === 0 && (
          <div style={{ textAlign:"center", color:sub, fontSize:13, padding:20 }}>
  ...
  return (
    <div style={{ background:pageBg, fontFamily:"Nunito,sans-serif", minHeight:"100vh" }}>
      {/* Header */}
      <div style={{ background:accent, color:"white", padding:"22px 16px 0",
        position:"sticky", top:0, zIndex:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
          <button onClick={onBack}
            style={{ background:"rgba(255,255,255,.2)", border:"none", borderRadius:50,
              color:"white", width:34, height:34, cursor:"pointer", fontSize:18, flexShrink:0,
              display:"flex", alignItems:"center", justifyContent:"center" }}>←</button>
          <div style={{ fontWeight:900, fontSize:18, flex:1 }}>💬 Chat Padres</div>
        </div>
        {/* Tabs */}
      ...
      {/* ── Sec 0: Global padres ──────────────────────────────── */}
      {sec === 0 && renderRoom(globalMsgs, "¡Sé el primero en escribir! 💬")}
          ...
          {groups.map(g => (
            <div key={g.conversation_id} onClick={() => openGroup(g)}
              style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px",
                cursor:"pointer", background:cardBg, borderRadius:16, marginBottom:8,
                border:`1px solid ${navBord}`, boxShadow:dark?"0 1px 8px rgba(0,0,0,.3)":"0 1px 8px rgba(0,0,0,.05)" }}>
      ...
      {/* ── Sec 2: Aula (padres del aula del hijo) ────────────── */}
      {sec === 2 && (
        classInfo
      ...
      {/* ── Sec 3: Privado — lista de contactos ───────────────── */}
      {sec === 3 && (
        <div style={{ padding:"12px 14px 80px" }}>
            ...
            <div key={f.friendship_id} onClick={() => openFriend(f)}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                {f.apodo && f.apodo !== f.nombre && (
                  <div style={{ fontSize:10, color:sub }}>({f.nombre})</div>
                )}
              </div>
        ...
        <div onClick={e => { if(e.target===e.currentTarget) setGroupModal(false); }}
          ...
          <div style={{ background:cardBg, borderRadius:"20px 20px 0 0",
            ...
            <div style={{ fontSize:11, fontWeight:800, color:sub, marginBottom:6 }}>
              AGREGAR CONTACTOS ({groupSel.size} seleccionados)
            </div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setGroupModal(false)}
                style={{ flex:1, background:inputBg, border:`1px solid ${navBord}`,
                  borderRadius:50, color:sub, padding:"13px", fontWeight:800, fontSize:14,
                  cursor:"pointer", fontFamily:"Nunito,sans-serif" }}>
                Cancelar
              </button>
