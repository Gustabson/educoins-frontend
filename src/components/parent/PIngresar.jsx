...
export default function PIngresar({ me, setTab }) {
  const { primary:accent, isDark:dark, txt, sub, cardBg, pageBg:bg } = useTheme();
  const [copied, setCopied] = useState(false);

  // CVU = primeros 22 chars del ID del usuario, formateado en grupos de 4
  ...
  return (
    <div style={{ minHeight:"100vh", background:bg, transition:"background .3s" }}>
      ...
      <div style={{ background:accent, color:"white", padding:"52px 20px 28px",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, position:"relative" }}>
          ...
          <div>
            <div style={{ fontWeight:900, fontSize:22 }}>⬇️ Ingresar</div>
            <div style={{ fontSize:13, opacity:.85, fontWeight:600, marginTop:2 }}>
      ...
      <div style={{ padding:"16px 14px", display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <div style={{ background:cardBg, borderRadius:24, padding:"24px 20px", textAlign:"center",
          ...
          <div style={{ fontSize:11, color:sub, marginTop:2 }}>
            ID: {me.id.slice(0,8).toUpperCase()}...
          </div>
        </div>
        ...
        <div style={{ background:cardBg, borderRadius:20, padding:"18px 16px",
          ...
          <button onClick={copiar} style={{
            ...
            boxShadow:`0 4px 14px ${copied?"#10b981":accent}44` }}>
        ...
        <div style={{ background:cardBg, borderRadius:20, padding:"16px",
          ...
          <div style={{ fontWeight:800, color:txt, fontSize:13, marginBottom:10 }}>
            ¿Cómo recibir monedas?
          </div>
          {[
            ["1","Compartí tu QR o tu ID con quien te quiera enviar monedas"],
            ["2","En la pantalla Enviar, la otra persona pega tu ID en la sección Manual"],
